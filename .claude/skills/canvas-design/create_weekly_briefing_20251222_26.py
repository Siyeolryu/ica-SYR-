...
# Date
date_text = "2025.12.22 - 12.26"
draw.text((WIDTH - 200, 35), date_text, fill=hex_to_rgb(TEXT_SECONDARY), font=font_date)
...
# Achievement 1: React Hydration Fix
card1_x = 40
...
draw.text((card1_x + 15, card_y_start + 15), "[Fix] React Hydration", fill=hex_to_rgb(SIGNAL_GREEN), font=font_heading)
draw.text((card1_x + 15, card_y_start + 50), "서버/클라이언트", fill=hex_to_rgb(TEXT_PRIMARY), font=font_body)
draw.text((card1_x + 15, card_y_start + 72), "렌더링 일치", fill=hex_to_rgb(TEXT_PRIMARY), font=font_body)
draw.text((card1_x + 15, card_y_start + 100), "useEffect 활용", fill=hex_to_rgb(TEXT_SECONDARY), font=font_small)
draw.text((card1_x + 15, card_y_start + 118), "NewsCard 수정", fill=hex_to_rgb(TEXT_SECONDARY), font=font_small)

# Achievement 2: CSS & Service Fix
...
draw.text((card2_x + 15, card_y_start + 15), "[Fix] CSS & Service", fill=hex_to_rgb(SIGNAL_GREEN), font=font_heading)
draw.text((card2_x + 15, card_y_start + 50), "border-3 → border-4", fill=hex_to_rgb(TEXT_PRIMARY), font=font_body)
draw.text((card2_x + 15, card_y_start + 72), "서비스 정상화", fill=hex_to_rgb(TEXT_PRIMARY), font=font_body)
draw.text((card2_x + 15, card_y_start + 94), "API 테스트 통과", fill=hex_to_rgb(TEXT_PRIMARY), font=font_body)
draw.text((card2_x + 15, card_y_start + 118), "프론트/백엔드 OK", fill=hex_to_rgb(TEXT_SECONDARY), font=font_small)
...
draw.text((card3_x + 15, card_y_start + 15), "[CI/CD] GitHub Actions", fill=hex_to_rgb(SIGNAL_GREEN), font=font_heading)
draw.text((card3_x + 15, card_y_start + 50), "워크플로우 개선", fill=hex_to_rgb(TEXT_PRIMARY), font=font_body)
draw.text((card3_x + 15, card_y_start + 72), "권한 설정 완료", fill=hex_to_rgb(TEXT_PRIMARY), font=font_body)
draw.text((card3_x + 15, card_y_start + 94), "스케줄: 월/수/금", fill=hex_to_rgb(TEXT_PRIMARY), font=font_body)
draw.text((card3_x + 15, card_y_start + 118), "19:00 KST 실행", fill=hex_to_rgb(TEXT_SECONDARY), font=font_small)
...
draw.text((600, tech_y), "React 18", fill=hex_to_rgb(TEXT_PRIMARY), font=font_small)
...
# Save the image
output_path = os.path.join('..', '..', '..', 'backend', 'output', 'reports', 'weekly_briefing_card_20251222_26.png')
